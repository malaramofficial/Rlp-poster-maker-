...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        root.addView(TextView(this).apply{text="फोटो को दाएं/बाएं करें"})
        root.addView(SeekBar(this).apply { max=100; progress=50; setOnSeekBarChangeListener(object:SeekBar.OnSeekBarChangeListener { override fun onProgressChanged(s:SeekBar?,p:Int,f:Boolean){ photoOffsetX=(p-50)*4f; render() }; override fun onStartTrackingTouch(s:SeekBar?){}; override fun onStopTrackingTouch(s:SeekBar?){} }) })
        root.addView(TextView(this).apply{text="फोटो को ऊपर/नीचे करें"})
        root.addView(SeekBar(this).apply { max=100; progress=50; setOnSeekBarChangeListener(object:SeekBar.OnSeekBarChangeListener { override fun onProgressChanged(s:SeekBar?,p:Int,f:Boolean){ photoOffsetY=(p-50)*4f; render() }; override fun onStartTrackingTouch(s:SeekBar?){}; override fun onStopTrackingTouch(s:SeekBar?){} }) })
    ...
    private fun createPoster():Bitmap{
        ...
        userBitmap?.let{src->
            ...
            c.save();c.clipPath(clip);c.drawBitmap(src,null,Rect((cx-radius+photoOffsetX).toInt(),(cy-radius+photoOffsetY).toInt(),(cx+radius+photoOffsetX).toInt(),(cy+radius+photoOffsetY).toInt()),null);c.restore()
